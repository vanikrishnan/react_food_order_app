...
function FoodItems() {
    ...
    useEffect(() => {
        axios.get('he-public-data/smartQFood8bef5a2.json')
        .then(response => {
            itemsDetails.dispatchItems({type: 'FETCH_SUCCESS', payload: response.data})
    ...
    return (
        ...
        <div className="container">
            ...
            {itemsDetails.itemsState.loading ? 'Loading' : itemsDetails.itemsState.items.map((item, index) => <RenderCard key={index} card={item} index={index}/>)}
